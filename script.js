  ...
  updateProgress() {
    ...
    for (let y = 0; y < this.gridSize; y++) {
      for (let x = 0; x < this.gridSize; x++) {
        if (!this.grid[y][x].blocked) {
          ...
          const expected = this.grid[y][x].letter.toUpperCase();
          const user = this.userInput[cellKey]?.toUpperCase();
    ...
    this.words.forEach((wordObj) => {
      ...
      for (let i = 0; i < word.length; i++) {
        ...
        if (
          this.userInput[`${cx},${cy}`]?.toUpperCase() !==
          this.grid[cy][cx].letter.toUpperCase()
        ) {
          complete = false;
          break;
        }
      ...
      if (complete) wordsFound++;
    });
  
    if (wordsFound === this.words.length && !this.gameCompleted) {
